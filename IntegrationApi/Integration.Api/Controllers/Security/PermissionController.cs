    ...
    public class PermissionController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetByFilter([FromHeader] HeaderDTO header, [FromQuery] string filterField, [FromQuery] string filterValue)
        {
            try
            {
                ...
                var result = await _service.GetByMultipleFiltersAsync(new List<Expression<Func<PermissionDTO, bool>>> { filter });
                return Ok(ResponseApi<IEnumerable<PermissionDTO>>.Success(result));
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> GetByMultipleFilters([FromHeader] HeaderDTO header, [FromQuery] Dictionary<string, string> filters)
        {
            try
            {
                ...
                var result = await _service.GetByMultipleFiltersAsync(new List<Expression<Func<PermissionDTO, bool>>> { filterExpression });
                return Ok(ResponseApi<IEnumerable<PermissionDTO>>.Success(result));
            }
            catch (Exception ex)
            {
